function [Offspring_resulted] = Mutation(c1,c2,Dimension,pm,evolving_pm_direction,vv,network,neighbourhoud,evaluated_offsprings,individual)
if evaluated_offsprings < individual
if (individual - evaluated_offsprings) == 1
%% -- Update the number of evaluated offsprings  --------------------------------
...
% ------------------------------------------------------------------
%% ---- Apply bit-flip to the third variant of offspring one -------
for n=1:Dimension
    if rand <= (pm*vv)
        c13(n) = 1 -  c13(n);
    end
...
% ------------------------------------------------------------------
%% ---- Apply bit-flip to the fourth variant of offspring one -------
for n=1:Dimension
    if rand <= pm
        c14(n) = 1 -  c14(n);
    end
...
% ------------------------------------------------------------------
%% ---- Apply bit-flip to the fourth variant of offsprings one -----
for n=1:Dimension
    if rand <= pm
        c14(n) = 1 -  c14(n);
    end
...
% ------------------------------------------------------------------
%% ---- Apply bit-flip mutation to the third variant of offspring # 2 ------------
for n=1:Dimension
    if rand <= (vv*pm)
        c23(n) = 1 -  c23(n);
    end
...
% ------------------------------------------------------------------
%% ---- Apply bit-flip mutation to the fourth variant of offspring # 2 ------------
for n=1:Dimension
    if rand <= pm
        c24(n) = 1 -  c24(n);
    end
