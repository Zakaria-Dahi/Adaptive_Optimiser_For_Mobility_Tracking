...
% Probability of crossover : onstnt always equal 1
% Mutation : Bit Flip Mutation
% Probability of Mutation : deterministic adaptive mutation probability using the constant declaining strategy
...
% Additional Information : record of statistical properties of the population at every time 
...
global execution 
       execution = [30 30 30 30 30 30 30 30 30 30 30 30];
%% -------------- The number of evaluations ----------------------------
global fitness_eval 
       fitness_eval = [175000 175000 175000 175000 175000 175000 175000 175000 175000 175000 175000 175000];
%% Save the best fitnesses reported by the state-of-the-art algorithm 
global state_of_the_art 
       state_of_the_art = [98535  97156 95038 173701 182331  174519 308401 287149  264204  386681 358167  371829];
...
%% Prameters  of the Genetic Algorithm  ----------------------
     ...
     pm = (0.5 - (1/Dimension))/2; % as a starting value we use a value in the middle of adviced intervals in the literature 1/L .. 0.5
     lambda = 1.1;
     vv = 1.5;
     segma = 0.5;
...
it = it + indiv;
% -------------------------------------------------------------------------
while it <= (iter - 8)% --------- itteration loop  ------------------------
% ------------ Calculating the remaining Allowable Estimation -------------
reste = iter - it; 
if reste > 0
if (floor(reste/4)) >= indiv 
   individual = indiv;
else
   individual = floor(reste/4);
end
...
%% ---------  No selection Process Is performed Because All the Population Will Undergo Crossover and Mutation -------------------------------       
  %% -------  Generate N/2 couple of chromosome parents by mean of binary tournament ---------------------------
   ...
   for k=1:ceil(individual/2)
            ...
            new_offspring = [];
            new_offsprings = Mutation(c1,c2,Dimension,pm,evolving_pm_direction,vv,network,neighbourhoud,evaluated_offsprings,individual);
            % --------------------------------------------------------------------------
