...
% Crossover :  use a HUX crossover that of the CHC algorithm
% Probability of crossover : constnt always equal 1
% Mutation : Bit Flip Mutation
% Probability of Mutation : deterministic adaptive mutation probability using the sinuosidal decreasing amplitude  : 0.3 - (1/L)
...
%% Prameters  of the Genetic Algorithm  ----------------------
     ...
     freq = 0.7; %% the frequency of the wave
...
%% variable used during the calcul ---------------------------------------
     ...
     %  Save the counter of fitness evaluations for the HUX crossover
     it = 0;
     distance = fix(Dimension/4);   
...
pm = pm_orig * (sind(2 * pi * freq * (it/indiv)) * (((iter/indiv) - (it/indiv))/(iter/indiv)) + 1);
...
%% ---------  No selection Process Is performed Because All the Population Will Undergo Crossover and Mutation -------------------------------       
  %% -----  Generate N/2 couple of chromosome parents by mean of binary tournament ---------------------------
   ...
   %% -------  The genetic crossover and genetic mutation  -----------------------------------
        for k=1:ceil(individual/2)
            %% ---- Apply the HUX crossover on the formed couples ----------------
            if rand <= pcr
             soustract = population(cr(k,1),:) - population(cr(k,2),:);
             ...
             if threshold > distance
                 ...
                 c1(differ(bit_flip)) = abs(c1(differ(bit_flip)) - 1);
                 c2(differ(bit_flip)) = abs(c2(differ(bit_flip)) - 1);
             end
            else
                c1 = population(cr(k,1),:);
                c2 = population(cr(k,2),:);
